...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let moveCount = 1; // Intialize the move count
  ...
  // Function to handle the start of a drag postion
  const onDragStart = (source, piece) => {
    // Allow the user to drag only their own pieces based on color
    return !game.game_over() && piece.search(userColor) === 0;
  };

  // Funtion to handle a pice drop on the board
  ...
  // Envent listener for the "Flip Board" button
  document.querySelector('.flip-board').addEventListener('click', () => {
    board.flip();
    makeRandomMove();
    // Toggle user's color after flippiing the board
